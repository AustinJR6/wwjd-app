...
// If you build an Express app elsewhere, ensure the webhook route uses bodyParser.raw({ type: 'application/json' }).
// For plain onRequest in Gen-1, Firebase provides req.rawBody already; DO NOT JSON-parse before constructEvent.
...
export const handleStripeWebhookV1 = functions.https.onRequest(async (req, res) => {
  ...
  try {
    // CRITICAL: use the raw body buffer to avoid "payload must be string/Buffer" + signature failures
    ...
    switch (event.type) {
      ...
      case 'invoice.payment_succeeded': {
        ...
        const userId =
          ...
          ((invoice as any).subscription_details?.metadata?.userId as string) ||
          '';
        if (userId) {
          await admin
            .firestore()
